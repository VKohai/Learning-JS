...
function calculator() {
  ...
  if (localStorage.getItem('sex')) sex = localStorage.getItem('sex');else sex = document.querySelector('#gender .calculating__choose-item_active').getAttribute('id');
  if (localStorage.getItem('ratio')) ratio = localStorage.getItem('ratio');else ratio = +document.querySelector('.calculating__choose_big .calculating__choose-item_active').getAttribute('data-ratio');
...
function cards() {
  ...
  (0,_services_services__WEBPACK_IMPORTED_MODULE_0__.getResoursesAsync)("http://localhost:3000/menu").then(cards => cards.forEach(card => new MenuCard(card.img, card.altimg, card.title, card.descr, card.price, ".menu .container").render()));
}
...
function forms(formSelector, modalTimerId) {
  ...
  function bindPostData(form) {
    form.addEventListener("submit", event => {
      ...
      // Making POST request
      (0,_services_services__WEBPACK_IMPORTED_MODULE_1__.postDataAsync)("http://localhost:3000/requests", formsJson).then(response => {
...
function modal(triggerSelector, modalSelector, modalTimerId) {
  ...
  modalTriggers.forEach(btn => btn.addEventListener("click", () => showModal(modalSelector, modalTimerId)));
  ...
  document.addEventListener("keydown", event => {
    if (event.code === "Escape" && modal.classList.contains("modal_showed")) {
      closeModal(modalSelector);
    }
  });
